...
public class Eclat {
	...
	public ArrayList<ArrayList<Set<Integer>>> calculate(int min_sup) {
		...
		for(int item : m_db.getIDs()) {											// (2)
			if( m_db.getClients(item).size() >= min_sup ) {						// (3)
		...
		while(found) {															// (7)
			...
			for( int idx1 = 0; idx1 < prev_iter_itemsets.size(); idx1++ ) {											// (9), (10)
				for( int idx2 = idx1 + 1; idx2 < prev_iter_itemsets.size(); idx2++ ) {								// (11), (12)
					Set<Integer> itemset1 = prev_iter_itemsets.get(idx1);											// (13)
					Set<Integer> itemset2 = prev_iter_itemsets.get(idx2);
					if(!itemset1.equals(itemset2) && compatible(itemset1, itemset2)) {								// (14), (15)
						ArrayList<Integer> new_tidlist = join(m_itemsets.get(itemset1), m_itemsets.get(itemset2));	// (16)
						Set<Integer> joined = new HashSet<Integer>(itemset1);
						joined.addAll(itemset2);
						if( new_tidlist.size() >= min_sup && !m_itemsets.containsKey(joined)) {						// (17), (18)
							m_found_itemsets.get(itemset_size).add(joined);											// (19)
	...
	private boolean compatible(Set<Integer> itemset1, Set<Integer> itemset2) {
		if( itemset1.size() == 1 && itemset2.size() == 1) {
			...
			tmp.retainAll(itemset2);									//alles wat ze gemeenschappelijk hebben
